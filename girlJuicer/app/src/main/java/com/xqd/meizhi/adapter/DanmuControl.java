...
import static com.xqd.meizhi.activity.base.BaseApplication.applicationContext;
...
public class DanmuControl {
    ...
    private static final long ADD_DANMU_TIME = 2000; //弹幕显示的时间(如果是list的话，会 * i)，记得加上mDanmakuView.getCurrentTime()
...
//            danmaku.padding = 20;  // 在背景绘制模式下增加padding
        ...
        public void drawBackground(BaseDanmaku danmaku, Canvas canvas, float left, float top) {
            ...
            if (!danmaku.isGuest && danmaku.userId == mGoodUserId && mGoodUserId != 0) {
                ...
                paint.setColor(mContext.getResources().getColor(R.color.green_1));//绿色 我
            } else {
                paint.setColor(mContext.getResources().getColor(R.color.black_tran));//黑色 普通
            }
            ...
            canvas.drawRoundRect(new RectF(left + DANMU_PADDING_INNER, top + DANMU_PADDING_INNER
                            ...
                            top + danmaku.paintHeight - DANMU_PADDING_INNER +4),//+6 主要是底部被截得太厉害了，+6是增加padding的效果
...
//            }
    ...
    public void addDanmu(Danmu danmu, int i) {
        ...
        try {
            myBitmap  = Glide.with(applicationContext)
                    .load(danmu.avatarUrl)
                    .asBitmap() //必须  
                    .centerCrop()
...
//        Bitmap myBitmap = getDefaultBitmap(danmu.avatarUrl);
